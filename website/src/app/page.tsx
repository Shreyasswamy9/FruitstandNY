...
function PhotoGroup({ hoveredNav }: PhotoGroupProps) {
  ...
  const lastNavRef = React.useRef<NavType | null>(null);
  if (hoveredNav) {
    lastNavRef.current = hoveredNav;
  }
  const photos = lastNavRef.current && navPhotos[lastNavRef.current as NavType] ? navPhotos[lastNavRef.current as NavType] : navPhotos["SHOP"];
  return (
...
const closeMenu = () => {
  ...
  return (
  <div style={{ position: "relative", minHeight: "100vh", width: "100vw", background: "#fff", zIndex: 9999, overflow: showMain ? "auto" : "hidden" }}>
      {/* Scrollable video section */}
      <div style={{ position: "relative", width: "100vw", height: "100vh", overflow: "hidden" }}>
        <motion.video
          ref={videoRef}
          id="intro-video"
          style={{ width: "100vw", height: "100vh", objectFit: "cover", position: "absolute", top: 0, left: 0, zIndex: 2, pointerEvents: "none" }}
          autoPlay
          muted
          playsInline
          preload="auto"
        ...
        <motion.video
          style={{ width: "100vw", height: "100vh", objectFit: "cover", position: "absolute", top: 0, left: 0, zIndex: 1, pointerEvents: "none" }}
          autoPlay
          muted
          playsInline
          preload="auto"
        ...
        {/* Header and menu overlay remain fixed */}
        {showMain && (
          <>
            <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10002, pointerEvents: "none", height: "80px" }}>
              <h1 style={{ color: "white", fontSize: "2.2rem", fontWeight: "bold", letterSpacing: "0.18em", textTransform: "uppercase", textAlign: "center", textShadow: "0 2px 8px #000, 0 0px 2px #000", margin: 0 }}>FRUITSTAND</h1>
            </div>
            ...
            {menuOpen && menuTransition === 'open' && (
              <motion.div
                ...
                style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", zIndex: 20002, display: "flex", background: "linear-gradient(120deg, #232323 0%, #b71c1c 100%)", transition: "background 0.7s ease" }}
              >
                ...
                <div style={{ width: 400, minWidth: 220, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-end", paddingRight: 60 }}>
                  {["SHOP", "ACCOUNT", "CART", "CONTACT"].map(nav => (
                    <button
                      key={nav}
                      style={{ margin: "18px 0", padding: "18px 38px", fontSize: 28, fontWeight: "bold", background: hoveredNav === nav ? "#fff" : "#f5f5f5", color: "#333", border: "none", borderRadius: 12, boxShadow: hoveredNav === nav ? "0 4px 24px #bbb" : "0 2px 8px #ccc", cursor: "pointer", transition: "all 0.3s" }}
      ...
      {/* Product grid only after intro video */}
      {showMain && (
        <div style={{ width: "100vw", background: "#f7f7f7", padding: "64px 0 120px 0", marginTop: 0 }}>
          <h2 style={{ fontSize: "2.5rem", fontWeight: "bold", marginBottom: 40, textAlign: "center", color: "#222", letterSpacing: "0.08em" }}>New Collection 2025</h2>
          <ProductsGrid />
        </div>
      )}
    </div>
