...
export default function GalaBundlePage() {
  ...
  const cameoTee = TEE_VARIANTS.find(t => t.slug === 'cameo-tshirt')!;
  
  const [bundleSize, setBundleSize] = useState<BundleQty>(1);
  const [items, setItems] = useState<BundleItem[]>([
    { color: cameoTee.colors[0], size: null },
  ]);
  ...
  const { isOutOfStock, isSizeSoldOut } = useProductStock('4e6ed8e5-0946-454c-bee4-b5048cac5b9b');

  useTrackProductView({
    productId: "4e6ed8e5-0946-454c-bee4-b5048cac5b9b",
    productName: PRODUCT.name,
    price: SHIRT_BUNDLE_PRICING.cameo[bundleSize],
    currency: "USD",
  });
  ...
  const handleBundleSizeChange = useCallback((newSize: BundleQty) => {
    ...
    setItems((prev) => {
      ...
      while (next.length < newSize) {
        next.push({ color: cameoTee.colors[0], size: null });
      }
  ...
  const addBundleToCart = useCallback(() => {
    if (!allFilled) return;
    const price = SHIRT_BUNDLE_PRICING.cameo[bundleSize];
    const summary = items
      .slice(0, bundleSize)
    ...
    addToCart({
      productId: "4e6ed8e5-0946-454c-bee4-b5048cac5b9b",
      name: `Cameo Tee Bundle (${bundleSize}) – ${summary}`,
      price,
      image: cameoTee.colors[0].image,
      quantity: 1,
    });
  }, [addToCart, bundleSize, allFilled, items, cameoTee.colors]);

  const boughtTogetherItems = useMemo(() => getFBTForPage("cameo-bundle"), []);

  const savingsInfo = useMemo(() => {
    if (bundleSize === 1) return null;
    const singlePrice = SHIRT_BUNDLE_PRICING.cameo[1] * bundleSize;
    const bundlePrice = SHIRT_BUNDLE_PRICING.cameo[bundleSize];
          ...
          <div className="mb-8 bg-white rounded-lg border-2 border-black/10 p-6">
            ...
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-[#1d1c19]">
                ${SHIRT_BUNDLE_PRICING.cameo[bundleSize]}
              </p>
              {savingsInfo && (
          ...
          {allFilled && (
            ...
            >
              <div className="max-w-4xl mx-auto">
                ...
                >
                  {isOutOfStock(null) ? 'OUT OF STOCK' : `Add to Cart • $${SHIRT_BUNDLE_PRICING.cameo[bundleSize]}`}
                </button>
              </div>
            </motion.div>
          )}
