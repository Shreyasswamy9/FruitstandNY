...
const PRODUCT = {
  ...
  details: [
    "All Mutsu Pocket Tees in this bundle",
...
export default function GalaBundlePage() {
  ...
  const mutsuTee = TEE_VARIANTS.find(t => t.slug === 'mutsu-tshirt')!;
  
  const [bundleSize, setBundleSize] = useState<BundleQty>(1);
  const [items, setItems] = useState<BundleItem[]>([
    { color: mutsuTee.colors[0], size: null },
  ]);
  ...
  useTrackProductView({
    ...
    price: SHIRT_BUNDLE_PRICING.mutsu[bundleSize],
    currency: "USD",
  });
  ...
  const handleBundleSizeChange = useCallback((newSize: BundleQty) => {
    ...
    setItems((prev) => {
      ...
      while (next.length < newSize) {
        next.push({ color: mutsuTee.colors[0], size: null });
      }
  ...
  const addBundleToCart = useCallback(() => {
    if (!allFilled) return;
    const price = SHIRT_BUNDLE_PRICING.mutsu[bundleSize];
    const summary = items
      .slice(0, bundleSize)
    ...
    addToCart({
      productId: "165899ab-6885-426b-9035-44ff75896522",
      name: `Mutsu Tee Bundle (${bundleSize}) – ${summary}`,
      price,
      image: mutsuTee.colors[0].image,
      quantity: 1,
    });
  }, [addToCart, bundleSize, allFilled, items, mutsuTee.colors]);

  const boughtTogetherItems = useMemo(() => getFBTForPage("mutsu-bundle"), []);

  const savingsInfo = useMemo(() => {
    if (bundleSize === 1) return null;
    const singlePrice = SHIRT_BUNDLE_PRICING.mutsu[1] * bundleSize;
    const bundlePrice = SHIRT_BUNDLE_PRICING.mutsu[bundleSize];
          ...
          <div className="mb-8 bg-white rounded-lg border-2 border-black/10 p-6">
            ...
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-[#1d1c19]">
                ${SHIRT_BUNDLE_PRICING.mutsu[bundleSize]}
              </p>
              {savingsInfo && (
