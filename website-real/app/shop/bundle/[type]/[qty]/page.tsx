...
type BundleTypeParam = 'gala' | 'mutsu' | 'cameo' | 'fuji';

const BUNDLE_CONFIG: Record<BundleTypeParam, {
  name: string;
  description: string;
  teeSlug: string;
  productId: string;
  ...
  mutsu: {
    ...
    description: "Mix and match your favorite Mutsu Tees. Select colors and sizes.",
    teeSlug: 'mutsu-tshirt',
    productId: '165899ab-6885-426b-9035-44ff75896522',
    fbtPage: 'mutsu-bundle',
    details: [
      "All Mutsu Tees in this bundle",
      "Select color + size for each tee",
      "Bundle pricing applied automatically",
      "Ships together as one curated set",
      "Quality guaranteed, Free returns."
  ...
  cameo: {
    ...
    description: "Mix and match your favorite Cameo Tees. Select colors and sizes.",
    teeSlug: 'cameo-tshirt',
    productId: '4e6ed8e5-0946-454c-bee4-b5048cac5b9b',
    fbtPage: 'cameo-bundle',
    details: [
      "All Cameo Tees in this bundle",
      "Select color + size for each tee",
      "Bundle pricing applied automatically",
      "Ships together as one curated set",
      "Quality guaranteed, Free returns."
  ...
  fuji: {
    ...
    description: "Mix and match your favorite Fuji Long Sleeves. Select colors and sizes.",
    teeSlug: 'fuji-tshirt',
    productId: 'a2faf1a9-461f-41b1-b6dd-8d3ec26b021c',
    fbtPage: 'fuji-bundle',
    details: [
      "All Fuji Long Sleeves in this bundle",
      "Select color + size for each item",
      "Bundle pricing applied automatically",
      "Ships together as one curated set",
      "Quality guaranteed, Free returns."
    ]
...
export default function DynamicBundlePage({
  ...
  params: Promise<{ type: string; qty: string }>;
}) {
  const router = useRouter();
  const { addToCart } = useCart();

  // Unwrap params promise
  const { type, qty } = React.use(params);

  // Validate parameters
  const bundleType = type.toLowerCase() as BundleTypeParam;
  const requestedQty = parseInt(qty, 10);

  // Check if valid bundle type and qty
  if (!Object.keys(BUNDLE_CONFIG).includes(bundleType) || isNaN(requestedQty) || requestedQty < 1 || requestedQty > 6) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Invalid Bundle Link</h1>
          <p className="text-gray-600 mb-4">This link appears to be invalid.</p>
          <button
            onClick={() => router.push('/shop')}
            className="px-6 py-3 bg-black text-white rounded-lg font-bold hover:bg-[#2a2a2a]"
          >
            Return to Shop
          </button>
        </div>
  ...
  const validQty = requestedQty as BundleQty;

  const teeVariant = TEE_VARIANTS.find(t => t.slug === config.teeSlug);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">The requested bundle could not be loaded.</p>
          <button
            onClick={() => router.push('/shop')}
            className="px-6 py-3 bg-black text-white rounded-lg font-bold hover:bg-[#2a2a2a]"
          >
  ...
  const [bundleSize, setBundleSize] = useState<BundleQty>(validQty);
  const [items, setItems] = useState<BundleItem[]>(() => {
    const items = [];
    for (let i = 0; i < validQty; i++) {
      // Distribute colors across tees - each tee gets a different color if available
      const colorIndex = i % teeVariant.colors.length;
      items.push({ color: teeVariant.colors[colorIndex], size: null });
    }
    return items;
  });
  ...
  const { isOutOfStock, isSizeSoldOut } = useProductStock(config.productId);

  useTrackProductView({
    productId: config.productId,
    productName: config.name,
    price: SHIRT_BUNDLE_PRICING[bundleType as keyof typeof SHIRT_BUNDLE_PRICING][bundleSize],
    currency: "USD",
  });
  ...
  const handleBundleSizeChange = useCallback((newSize: BundleQty) => {
    ...
    setItems((prev) => {
      ...
      while (next.length < newSize) {
        // Distribute colors across tees
        const colorIndex = next.length % teeVariant.colors.length;
        next.push({ color: teeVariant.colors[colorIndex], size: null });
      }
  ...
  const addBundleToCart = useCallback(() => {
    if (!allFilled) return;
    const bundleTypePricing = SHIRT_BUNDLE_PRICING[bundleType as keyof typeof SHIRT_BUNDLE_PRICING];
    const price = bundleTypePricing[bundleSize];
    const summary = items
      .slice(0, bundleSize)
    ...
    addToCart({
      productId: config.productId,
      name: `${config.name} (${bundleSize}) – ${summary}`,
      price,
      image: teeVariant.colors[0].image,
      quantity: 1,
    });
  }, [addToCart, bundleSize, allFilled, items, bundleType, teeVariant.colors, config]);

  const boughtTogetherItems = useMemo(() => getFBTForPage(config.fbtPage), [config.fbtPage]);

  const savingsInfo = useMemo(() => {
    if (bundleSize === 1) return null;
    const bundleTypePricing = SHIRT_BUNDLE_PRICING[bundleType as keyof typeof SHIRT_BUNDLE_PRICING];
    const singlePrice = bundleTypePricing[1] * bundleSize;
    const bundlePrice = bundleTypePricing[bundleSize];
          ...
          <div className="mb-8 bg-white rounded-lg border-2 border-black/10 p-6">
            ...
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-[#1d1c19]">
                ${SHIRT_BUNDLE_PRICING[bundleType as keyof typeof SHIRT_BUNDLE_PRICING][bundleSize]}
              </p>
              {savingsInfo && (
                  ...
                  <div className="px-6 py-6 max-w-2xl mx-auto">
                    ...
                    <div className="mb-8">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        {editingTeeIndex !== null && (
                          <p className="text-xs text-green-700 font-semibold">
                            💡 Suggested: {teeVariant.colors[editingTeeIndex % teeVariant.colors.length].name}
                          </p>
                        )}
                      </div>
                      <div className="grid grid-cols-5 sm:grid-cols-6 gap-3">
                        {teeVariant.colors.map((color, colorIndex) => {
                          const isRecommended = colorIndex === (editingTeeIndex !== null ? editingTeeIndex % teeVariant.colors.length : -1);
                          return (
                            <button
                              key={color.name}
                              onClick={() => setTempColor(color)}
                              className={`aspect-square rounded-lg border-3 transition-all relative ${
          ...
          {allFilled && (
            ...
            >
              <div className="max-w-4xl mx-auto">
                ...
                >
                  {isOutOfStock(null) ? 'OUT OF STOCK' : `Add to Cart • $${SHIRT_BUNDLE_PRICING[bundleType as keyof typeof SHIRT_BUNDLE_PRICING][bundleSize]}`}
                </button>
              </div>
            </motion.div>
          )}
