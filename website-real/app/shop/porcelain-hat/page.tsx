...
export default function PorcelainHatPage() {
  ...
  const boughtTogetherItems = [
    { id: 'mandarin-tee', name: 'Mandarin Tee', price: 38, image: '/images/products/Mandarin Tee/Mandarin 1.png' },
  ...
  <div className="flex flex-col md:flex-row gap-8 max-w-4xl mx-auto py-12 px-4" style={{ paddingTop: 120, paddingBottom: taskbarHeight }}>
        <div className="flex w-full md:w-1/2 flex-col items-center gap-4">
          ...
          <div className="flex gap-2 mt-4">
            {porcelainHatImages.map((img, idx) => (
              <button
                key={img}
              ...
              >
                <Image src={img} alt={`Porcelain Hat ${idx + 1}`} width={56} height={56} style={{ objectFit: 'cover', borderRadius: 8 }} />
              </button>
            ))}
          </div>
        </div>
